/*
 * Copyright (c) 2021-2026 See AUTHORS file.
 *
 ...
 * with emoji (see {@link KnownFonts#addEmoji(Font)}, which calls {@link #addAtlas(TextureAtlas)} here). There are some
 * other new features, like the option to draw outlines around text, or to add a drop shadow. Almost everything this
 * class can do is configurable, such as the {@link #boldStrength} to change the weight of bold text, or the
 * {@link #obliqueStrength} to change the angle of oblique text.
 * <br>
 ...
 * SDF typically looks the same as MSDF at all but the largest sizes, but enables either the standard smooth lines or an
 * {@link DistanceFieldType#SDF_OUTLINE outlined mode} to be used. "Standard" fonts look the best when mixing different
 * Fonts in one piece of text. Standard fonts usually need the smallest PNG files, SDF in second, and MSDF the largest.
 * <br>
 * While you can generate SDF fonts with Hiero or
 ...
 * that is part of libGDX; MSDF fonts need a different tool.
 * <a href="https://github.com/tommyettinger/fontwriter">fontwriter</a> is strongly recommended, and is used for all
 * fonts distributed in TextraTypist's {@link KnownFonts} except for some specific bitmap fonts. The metrics produced by
 * fontwriter tend to be significantly more accurate by default than ones produced by Hiero, especially for SDF fonts.
 * In older versions of TextraTypist, adjusting metrics like {@link #descent} was necessary for Hiero-produced fonts;
 * the KnownFonts in version 2.x and are mostly generated by FontWriter and rarely need configuration for each metric.
 * <br>
 * Using fontwriter means you get Structured JSON Font files instead of AngelCode BMFont files; these can be loaded with
 * {@link #Font(String, TextureRegion, float, float, float, float, boolean, boolean)} (the last parameter for any
 * constructor that only loads Structured JSON is called "ignoredStructuredJsonFlag"). You can also use
 * {@link Font#Font(FileHandle)} with any FileHandle for a .json, .json.lzma, or other Structured JSON file extension;
 * it will switch between loading an AngelCode BMFont or a Structured JSON font based on the file extension.
 * Structured JSON Fonts can be SDF, MSDF, standard (no distance field), or some other kinds of font; the information
 * about this is stored in the font. You can also load a libGDX BitmapFont with a Structured JSON Font using the
 * {@link BitmapFontSupport} class or the
 * <a href="https://github.com/tommyettinger/BitmapFontBridge">BitmapFontBridge</a> external library if you don't use
 * TextraTypist. Structured JSON files are often compressed because they have a repetitive structure that compresses
 * well. The earliest method for this produced ".dat" fonts using {@link LZBCompression}, but better compression
 ...
 * a Layout. The Layout is usually stored inside a {@link TextraLabel} or {@link TypingLabel}, available by
 * the {@link TextraLabel#layout} field or the {@link TypingLabel#getWorkingLayout()} method for the subtly-different
 * "working" Layout that a TypingLabel adds to over time. You can also render a Layout on your own using methods in this
 * class, but that gives up some features. Rendering a Layout directly is mostly similar to using a TextraLabel; a
 * TypingLabel has significantly more functionality.
 * <br>
 ...
 * a block of text with rotation, you should create a TextraLabel instead, put it in a {@link TransformContainer}
 * and rotate that using {@link TransformContainer#setRotation(float)} or any scene2d
 * {@link com.badlogic.gdx.scenes.scene2d.Action} that changes rotation.
 * <br>
 * Rotated text looks a lot better using a transformed Group such as a (Transform)Container, and if you're using a
 ...
 * <br>
 * There are some features here that cannot be used purely from markup, such as per-character rotation, but these can
 * be used by TypingLabel and its Effect assortment.
 *
...
//        fancyY = 2f;
                            ...
                            float xAdjust, float yAdjust, float widthAdjust, float heightAdjust, boolean makeGridGlyphs)
    {
        ...
        } else if ("ubj".equalsIgnoreCase(jsonHandle.extension())) {
    ...
    protected void drawBlockSequence(Batch batch, float[] sequence, TextureRegion block, float color, float x, float y,
        ...
        for (int b = 0; b < sequence.length; b += 4) {
            ...
            if(breadth != 1f){
                ...
                float thinDelta = (thinAcross - BlockUtils.THIN_ACROSS) * 0.5f;
                float wideDelta = (wideAcross - BlockUtils.WIDE_ACROSS) * 0.5f;
                ...
                if(sizeX == BlockUtils.THIN_ACROSS || sizeX == BlockUtils.TWIN_ACROSS) sizeX = thinAcross;
                    ...
                    if (sizeX == BlockUtils.THIN_OVER || sizeX == BlockUtils.TWIN_OVER1 || sizeX == BlockUtils.TWIN_OVER2) sizeX += thinDelta;
                    else if (sizeX == BlockUtils.WIDE_OVER) sizeX += wideDelta;
                } else if(startX > 0f) {
                    if (sizeX == BlockUtils.THIN_OVER || sizeX == BlockUtils.TWIN_OVER1 || sizeX == BlockUtils.TWIN_OVER2) sizeX += thinDelta;
                    else if (sizeX == BlockUtils.WIDE_OVER) sizeX += wideDelta;
                }
                ...
                if(sizeY == BlockUtils.THIN_ACROSS || sizeY == BlockUtils.TWIN_ACROSS) sizeY = thinAcross;
                    ...
                    if (sizeY == BlockUtils.THIN_OVER || sizeY == BlockUtils.TWIN_OVER1 || sizeY == BlockUtils.TWIN_OVER2) sizeY += thinDelta;
                    else if (sizeY == BlockUtils.WIDE_OVER) sizeY += wideDelta;
                } else if(startY > 0f) {
                    if (sizeY == BlockUtils.THIN_OVER || sizeY == BlockUtils.TWIN_OVER1 || sizeY == BlockUtils.TWIN_OVER2) sizeY += thinDelta;
                    else if (sizeY == BlockUtils.WIDE_OVER) sizeY += wideDelta;
                }

                if(startX == BlockUtils.THIN_START || startX == BlockUtils.TWIN_START1 || startX == BlockUtils.TWIN_START2) startX -= thinDelta;
                else if(startX == BlockUtils.WIDE_START) startX -= wideDelta;

                if(startY == BlockUtils.THIN_START || startY == BlockUtils.TWIN_START1 || startY == BlockUtils.TWIN_START2) startY -= thinDelta;
                else if(startY == BlockUtils.WIDE_START) startY -= wideDelta;
            }
        ...
        if (Float.isNaN(tr.offsetX)) {
            ...
            if(((glyph & ALTERNATE_MODES_MASK) == HALO) || ((glyph & ALTERNATE_MODES_MASK) == NEON)) {
                for (int xi = 3; xi >= 1; xi--) {
                    drawBlockSequence(batch,
                            (c <= 0x250B || (c >= 0x254C && c <= 0x254F)) ? BlockUtils.BOX_DRAWING[c & 3] : boxes,
                            font.mapping.get(solidBlock, tr),
            ...
            if (under != null && Float.isNaN(under.offsetX)) {
                ...
                if(((glyph & ALTERNATE_MODES_MASK) == HALO) || ((glyph & ALTERNATE_MODES_MASK) == NEON)) {
                    ...
                    drawBlockSequence(batch, BlockUtils.BOX_DRAWING[0], font.mapping.get(solidBlock, tr),
                            ...
                            x + (cos * p0x - sin * p0y), y + (sin * p0x + cos * p0y),
                            (changedW * (font.underLength + 1.1f)),
                            font.cellHeight * sizingY * (1.75f + font.underBreadth), rotation);
                }
            ...
            if (dash != null && Float.isNaN(dash.offsetX)) {
                ...
                if(((glyph & ALTERNATE_MODES_MASK) == HALO) || ((glyph & ALTERNATE_MODES_MASK) == NEON)) {
                    ...
                    drawBlockSequence(batch, BlockUtils.BOX_DRAWING[0], font.mapping.get(solidBlock, tr),
                            ...
                            x + (cos * p0x - sin * p0y), y + (sin * p0x + cos * p0y),
                            (changedW * (font.strikeLength + 1.1f)),
                            font.cellHeight * sizingY * (1.75f + font.strikeBreadth), rotation);
                }
