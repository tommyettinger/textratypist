 ...
 * {@link TypingLabel#setSelectable(boolean) selectable} set to true. This can use a {@link TypingListener} that
 * checks for the event {@code "*SELECTED"}, and calls {@link TypingLabel#copySelectedText()} when that event is
 * received. Or, call {@link TypingLabel#copySelectedText()} when the user requests, such as by pressing Ctrl-c; this
 * doesn't need a TypingListener.
 * <br>
 * The preferred height of a text field is the height of the {@link Styles.TextFieldStyle#font} and
 * {@link Styles.TextFieldStyle#background}. The preferred width of a text field is 150, a relatively arbitrary size.
 * <br>
 ...
 * standard implementations, one for the desktop and one for Android.
 * <br>
 * This allows entering emoji via paste, and displays them correctly if emoji are present in the Font used (such as
 * with {@link KnownFonts#addEmoji(Font)}). It will not handle emoji entered via typing or other char-at-a-time input
 * methods, at least on desktop. LWJGL3 and GLFW handle emoji inputs by entering one or more UTF-32 codepoints per
 * entered emoji. The problem is, libGDX's {@link InputListener#keyTyped(InputEvent, char)} only gives the program a
 * UTF-16 {@code char} type, and truncates off any extra information in a UTF-32 codepoint. Emoji can also be copied
 * from a TextraField, and will paste as the correct form for other applications to display (not the internal format
 * using PUA chars that TextraTypist uses).
 * <br>
...
//			displayText = passwordBuffer.toString();
	...
	protected int delete (boolean fireChangeEvent) {
		...
		if(glyphs.size > 0 && label.selectionEnd >= 0 && label.selectionStart <= label.selectionEnd)
			glyphs.removeRange(label.selectionStart, Math.max(Math.min(glyphs.size - 1, label.selectionEnd), 0));
		if (fireChangeEvent)
