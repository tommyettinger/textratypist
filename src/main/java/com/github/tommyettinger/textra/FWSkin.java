 ...
 * FontWriter's JSON format into either {@link Font} and {@link BitmapFont} objects, from the same files. These files
 * can have a variety of extensions depending on if or how they are compressed: .json, .dat, .ubj, .json.lzma, or
 * .ubj.lzma ; .ubj.lzma compresses best, with .json.lzma close behind, and for GWT compatibility reasons, .json.lzma is
 * currently preferred. It can also load {@link Font}s from AngelCode BMFont files (with a ".fnt" extension), and
 * continues to be able to load {@link BitmapFont}s from those files as well.
 * <br>
 * This class is also important when using the styles in {@link Styles}, since it allows reading in a skin JSON's styles
 * both as the scene2d.ui format and as styles for TextraTypist widgets. Because this extends Skin, you can and should
 * use FWSkin or an extension of it in place of a Skin object when working with TextraTypist (even if you only use .fnt
 * fonts). If you are using {@link com.badlogic.gdx.assets.AssetManager}, use {@link FWSkinLoader}. If you need to load
 * FreeType font parameters from a skin, use the related
 * <a href="https://github.com/tommyettinger/freetypist">FreeTypist</a> dependency and its {@code FreeTypistSkin} or
 * {@code FreeTypistSkinLoader}.
 * <br>
 ...
 * jagged or aliased. It only works on Font objects that use a distance field (SDF, MSDF, or SDF_OUTLINE). If you are
 * using a Viewport (which is strongly recommended for any production code in libGDX), then you should prefer
 * {@link #resizeDistanceFields(float, float, Viewport)} to give more information to the Font items being resized.
 */
