...
//        text.append("Imagine the possibilities! =D");
        ...
        text.append("{SLOWER}{GRADIENT=FF70F1;light exciting pink orange of justice;-0.5;5}{EASE=-8;2;1}{SHRINK=2;5}[%125][@Medieval]Welcome,[%]{ENDSHRINK}[@] {WAIT}" +
                "{SPIRAL=2;0.5;-2.5}{STYLE=/}{STYLE=;}[%?3]{VAR=title}[%]{STYLE=;}{STYLE=/}{ENDSPIRAL}![] {TRIGGER=lightest violet}[lightest violet][_][+🤔][]{WAIT=0.8}");
        text.append("{FAST}\n\n");
        text.append("{RESET}[@Sans]{ATTENTION}This is a [*][MAROON][%?8]simple[%?][WHITE][*] test[@]{ENDATTENTION} to {SPIN}show you{ENDSPIN}");
        ...
        text.append("{RAINBOW=1;1;0.7} [%?10]jussst[%?] to [%?12]catching[%?] an [%?0]event in [%?14]code[%?]{EVENT=example}!{WAIT} {ENDRAINBOW}");
        text.append("{NORMAL}\n\n");
