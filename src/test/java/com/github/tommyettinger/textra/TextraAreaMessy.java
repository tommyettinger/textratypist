 ...
 * */
public class TextraAreaMessy extends TextraField {
	...
	public TextraAreaMessy(String text, Skin skin) {
		super(text, skin);
	}

	public TextraAreaMessy(String text, Skin skin, String styleName) {
		super(text, skin, styleName);
	}

	public TextraAreaMessy(String text, TextFieldStyle style) {
		super(text, style);
	}
...
//		setAlignment(Align.topLeft);
	...
	public float getPrefHeight () {
		if (prefRows <= 0) {
			...
			float prefHeight = MathUtils.ceil(style.font.cellHeight * prefRows);
...
//				Font.Glyph lineFirst = fontData.getGlyph(displayText.charAt(lineStart));
//				if (lineFirst != null) {
//					// see FontData.getGlyphs()#852 for offset calculation
...
//				}
				...
				selection.draw(batch, x + selectionX + fontLineOffsetX, y - lineHeight - offsetY,
					selectionWidth + fontLineOffsetWidth, font.cellHeight);
			}

			offsetY += font.cellHeight;
			i += 2;
		}
	}

	protected void drawCursor (Drawable cursorPatch, Batch batch, Font font, float x, float y) {
		cursorPatch.draw(batch, x + getCursorX(), y + getCursorY(), cursorPatch.getMinWidth(), font.cellHeight);
	}
	...
	protected void calculateOffsets () {
		...
		if (!this.text.equals(lastText)) {
			...
			for (int i = 0; i < layout.lines(); i++) {
				...
				linesBreak.add(line.glyphs.size);
				lineStart += line.glyphs.size;
			}
...
//			}
	...
	public float getCursorY () {
		Font font = style.font;
		return -(cursorLine - firstLineShowing + 1) * font.cellHeight;
	}
	...
	public class TextAreaListener extends TextFieldClickListener {
		protected void setCursorPosition (float x, float y) {
			...
			cursorLine = (int)Math.floor((height - y) / font.cellHeight) + firstLineShowing;
