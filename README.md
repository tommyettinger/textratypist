...
```groovy
implementation "com.github.tommyettinger:textratypist:2.4.2"
```
...
the same; the only change should be going from 2.4.0 (or older) to 2.4.2 in a typical project. If you didn't create your
...
```groovy
implementation "com.github.tommyettinger:textratypist:2.4.2:sources"
...
If you need compatibility with libGDX 1.13.1, change `2.4.2` to `2.1.11`; it should have feature parity with `2.2.9`.
...
(Now, FreeTypist 2.4.2.0 uses TextraTypist 2.4.2, and that means it uses libGDX 1.14.2 . There's also FreeTypist
...
```groovy
implementation "com.github.tommyettinger:freetypist:2.4.2.0:sources"
```
...
Version 2.1.0 through 2.4.2 are out, and while they have fewer breaking changes, there are still several of them.
...
The 2.3.x line depends on libGDX 1.14.1, because that release broke a small but non-zero amount of TextraTypist code in
...
The quick checklist for the latest code (version 2.4.2 or newer commits from JitPack):
